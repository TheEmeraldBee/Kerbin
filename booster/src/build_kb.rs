...
pub fn generate_lib_rs(plugins: &[Plugin]) -> String {
    ...
    lib.push_str("use kerbin_core::{CommandRegistry, State};\n");
    lib.push('\n');
    ...
    lib.push_str("pub fn register_commands(registry: &mut CommandRegistry) {\n");
    lib.push_str("    kerbin_core::register_core_commands(registry);\n");
    for plugin in plugins {
        lib.push_str(&format!("    {}::register_commands(registry);\n", plugin.rust_name));
    }
    lib.push_str("}\n");
